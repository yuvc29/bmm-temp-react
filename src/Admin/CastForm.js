...
const CastForm = ({movieId}) =>{
  ...
  function handleClick() {
    ...
    const fetchData = async()=>{
      try{
        const response = await axios.post(`http://localhost:8080/cast/movie?movieId=${movieId}&actorIds=${idString}`, {movieId:fields.movieId, actorIds:idString})
        const data = response.data;
        console.log("Cast Post Data " ,data);
        alert("Cast added")
        // .then(response => response.json())
      }
      catch(err){
